    ...
    for env_name in ["diamor:corridor"]:
        ...
        for day in days:
            ...
            for group in tqdm(groups):
                ...
                dict_sinuosity["group"][group_id] = {
                    "group sinuosity": [],
                    "social_binding": soc_binding,
                    "encounters sinuosity": [],
                }
                ...
                for non_group in group_encounters:
                    ...
                    ng_sinuosity = {"sinuosity": float, "mean_velocity": np.ndarray, "length_of_trajectory": float, "time": float}

                    length_non_group = compute_length(traj_non_group_vicinity)
                    ng_sinuosity["sinuosity"] = compute_sinuosity(traj_non_group_vicinity)

                    group_sinuosity = {"sinuosity": float, "mean_velocity": np.ndarray, "length_of_trajectory": float, "time": float}

                    length_group = compute_length(traj_group_vicinity)
                    group_sinuosity["sinuosity"] = compute_sinuosity(traj_group_vicinity)

                    if(PLOT_VERIF) :
                        fig, ax = plot_baseline(trajectory = traj_group_vicinity , max_dev = group_sinuosity,soc_binding = soc_binding,group = True, id = group_id, boundaries = env.boundaries, colors = colors,ax = None,
                                            n_average = N_POINTS_AVERAGE, show = False)
                        plot_baseline(trajectory = traj_non_group_vicinity , max_dev = ng_sinuosity,soc_binding = soc_binding,group = False, id = non_group_id, boundaries = env.boundaries, colors = colors, ax = ax, fig = fig,
                                    show = True
                        )
                        plt.close()

                    group_sinuosity["mean_velocity"] = mean_group_speed
                    ng_sinuosity["mean_velocity"] = encounter_speed

                    group_sinuosity["time"] = time_of_group_traj
                    ng_sinuosity["time"] = time_of_non_group_traj

                    group_sinuosity["length_of_trajectory"] = length_group
                    ng_sinuosity["length_of_trajectory"] = length_non_group

                    dict_sinuosity["group"][group_id]["group sinuosity"].append(group_sinuosity)
                    dict_sinuosity["group"][group_id]["encounters sinuosity"].append(ng_sinuosity)


    pickle_save(f"../data/pickle/{env_name_short}_encounters_sinuosity.pkl", dict_sinuosity)
