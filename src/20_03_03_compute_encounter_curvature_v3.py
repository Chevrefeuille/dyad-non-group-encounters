    ...
    for env_name in ["diamor:corridor"]:
        ...
        for day in days:
            ...
            all_pedestrians = env.get_pedestrians(
                days=[day], no_groups=False, thresholds=thresholds_indiv, sampling_time=100
            )
            ...
            for group in tqdm(groups):
                ...
                dict_curvature["group"][group_id] = {
                    "group curvature": [],
                    "social_binding": soc_binding,
                    "encounters curvature": [],
                }
                ...
                for non_group in group_encounters:
                    ...
                    ng_curvature = {"curvature": float, "mean_velocity": np.ndarray, "length_of_trajectory": float, "time": float}

                    length_non_group = compute_length(traj_non_group_vicinity)
                    ng_curv = compute_curvature_v3(traj_non_group_vicinity)

                    group_curvature = {"curvature": float, "mean_velocity": np.ndarray, "length_of_trajectory": float, "time": float}

                    length_group = compute_length(traj_group_vicinity)
                    group_curv = compute_curvature_v3(traj_group_vicinity)

                    if(PLOT_VERIF) :
                        fig, ax = plot_baseline(trajectory = traj_group_vicinity , max_dev = group_curvature,soc_binding = soc_binding,group = True, id = group_id, boundaries = env.boundaries, colors = colors,ax = None,
                                            n_average = N_POINTS_AVERAGE, show = False)
                        plot_baseline(trajectory = traj_non_group_vicinity , max_dev = ng_curvature,soc_binding = soc_binding,group = False, id = non_group_id, boundaries = env.boundaries, colors = colors, ax = ax, fig = fig,
                                    show = True
                        )
                        plt.close()
                    ...
                    group_curvature["curvature_max"] = np.nanmax(group_curv)
                    ng_curvature["curvature_max"] = np.nanmax(ng_curv)

                    group_curvature["mean_velocity"] = mean_group_speed
                    ng_curvature["mean_velocity"] = encounter_speed

                    group_curvature["time"] = time_of_group_traj
                    ng_curvature["time"] = time_of_non_group_traj

                    group_curvature["length_of_trajectory"] = length_group
                    ng_curvature["length_of_trajectory"] = length_non_group

                    dict_curvature["group"][group_id]["group curvature"].append(group_curvature)
                    dict_curvature["group"][group_id]["encounters curvature"].append(ng_curvature)


    pickle_save(f"../data/pickle/{env_name_short}_encounters_curvature_v3{SAMPLING_NUMBER}.pkl", dict_curvature)
