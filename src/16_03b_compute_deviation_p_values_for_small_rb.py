...
if __name__ == "__main__":
    ...
    for env_name in ["diamor:corridor"]:
        ...
        group_sizes = pickle_load(f"../data/pickle/group_size_{env_name_short}.pkl")

        intensities = [0, 1, 2, 3]
        threshold_rb = 1.5

        for i in intensities:
            scaled_rb = rbs[i] / np.nanmean(group_sizes[i])
            indx_small_rb = scaled_rb < threshold_rb
            deviations[i]["ext"] = np.array(deviations[i]["ext"])[
                indx_small_rb
            ].tolist()
            deviations[i]["int"] = np.array(deviations[i]["int"])[
                indx_small_rb
            ].tolist()
            deviations[i]["group"] = np.array(deviations[i]["group"])[
                indx_small_rb
            ].tolist()
            deviations[i]["non_group"] = np.array(deviations[i]["non_group"])[
                indx_small_rb
            ].tolist()
