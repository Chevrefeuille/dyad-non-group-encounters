    ...
    for env_name in ["atc", "diamor"]:
        ...
        for day in days:
            ...
            for group in groups:
                ...
                # print(group_times_undisturbed)

                A_trajectory, B_trajectory = get_trajectories_at_times(
                    [m.get_trajectory() for m in group.get_members()],
                    group_times_undisturbed,
                )

                d_AB = compute_interpersonal_distance(
                    A_trajectory[:, 1:3], B_trajectory[:, 1:3]
                )

                breadth[soc_binding] += list(d_AB)

        pickle_save(
            f"../data/pickle/group_breadth_without_interaction_{env_name}.pkl", breadth
        )
        # plt.plot(d_AB)
        # plt.show()
