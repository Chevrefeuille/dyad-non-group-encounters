...
if __name__ == "__main__":
    ...
    for env_name in ["atc:corridor", "diamor:corridor"]:
        ...
        data_bin = np.empty((len(bin_centers), 2 * len(soc_binding_values) + 2))
        data_fit = np.empty((len(fit_x), 2 + len(soc_binding_values)))
        data_fit[:, 0] = fit_x
        ...
        data_bin[
            :, 2 * len(soc_binding_values) : 2 * len(soc_binding_values) + 1
        ] = np.array(r0s)[..., None]
        data_bin[
            :, 2 * len(soc_binding_values) + 1 : 2 * len(soc_binding_values) + 2
